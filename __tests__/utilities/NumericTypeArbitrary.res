...
let int8Arb = () => rawInt8Arb()->Derive.map(int8Exn)

let rawUint8Arb = () => integerRange(0, 255)
let uint8Arb = () => rawUint8Arb()->Derive.map(uint8Exn)

let rawInt16Arb = () => integerRange(-32768, 32767)
let int16Arb = () => rawInt16Arb()->Derive.map(int16Exn)

let int32Arb = () => integer()

let rawUint16Arb = () => integerRange(0, 65535)
let uint16Arb = () => rawUint16Arb()->Derive.map(uint16Exn)

let rawUint32Arb = () => floatRange(0.0, 4294967295.0)->Derive.map(Js.Math.round)
let uint32Arb = () => rawUint32Arb()->Derive.map(uint32Exn)
...
let uint64Arb = () => rawUint64Arb()->Derive.map(biguint64Exn)
...
let int64Arb = () => rawInt64Arb()->Derive.map(bigint64Exn)

let float32Arb = () => float_()->Derive.map(float32Exn)
